...
export async function renderEquirectangular(
  ...
  const weightSum = new Float32Array(outWidth * outHeight);
  ...
  for (let s = 0; s < shots.length; s++) {
    ...
    for (let row = rowStart; row <= rowEnd; row++) {
      for (let c = 0; c < colCount; c++) {
        ...
        accum[outIdx * 3 + 0] += r * shot.exposureGain * weight;
        accum[outIdx * 3 + 1] += g * shot.exposureGain * weight;
        accum[outIdx * 3 + 2] += b * shot.exposureGain * weight;
  ...
  fillUncoveredPoleCap(data, weightSum, outWidth, outHeight, 'top');
  fillUncoveredPoleCap(data, weightSum, outWidth, outHeight, 'bottom');
...
export function fillUncoveredPoleCap(
  ...
  for (const row of rowSequence) {
    ...
    for (let x = 0; x < width; x++) if (weightSum[base + x] > 1e-6) covered++;
  ...
  for (let row = capStart; row < capEnd; row++) {
    for (let x = 0; x < width; x++) {
      ...
      if (weightSum[idx] > 1e-6) continue;
